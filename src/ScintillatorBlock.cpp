/*
**	Filename : ScintillatorBlock.cpp
**	2024-02-13
**	username : rsehgal
*/
#include <iostream>
...
ScintillatorBlock::ScintillatorBlock(G4String name, double scintHalfX, double scintHalfY, double scintHalfZ,
      ...
      fPmtDz(pmtDz)
{
  ...
  // ⁠ Applying 90 deg rotation aroundZ axis
  rotation->rotateY(90 * deg);

  fScintillatorEnvelopeHalfX = fScintHalfX + (2 * fPmtDz);
  ...
  new G4PVPlacement(rotation,                                    // no rotation
                    G4ThreeVector(fScintHalfX + fPmtDz, 0., 0.), //
                    logicalPMT,                                  // its logical volume
                    "PhysicalPMT",                               // its name
                    fLogicalVolume,                              // its mother  volume
                    false,                                       // no boolean operation
                    ...
                    checkOverlaps);
  new G4PVPlacement(rotation,                                            // no rotation
                    G4ThreeVector(-1. * (fScintHalfZ + fPmtDz), 0., 0.), //
                    logicalPMT,                                          // its logical volume
                    "PhysicalPMT",                                       // its name
                    fLogicalVolume,                                      // its mother  volume
                    false,                                               // no boolean operation
