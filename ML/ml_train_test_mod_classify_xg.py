...
def load_classfication_data(file_path, tree_name, num_events):
    tree = uproot.open(file_path)[tree_name]
    df = tree.arrays(library="pd")
    df_shuffled = df.sample(frac=1).reset_index(drop=True)
    print(df_shuffled["pocaX"])
...
# Load training data
#X_train, y_train, output_keys = load_classfication_data(background_file_path, tree_name, num_background_events)
#X_train, y_train, output_keys = load_classfication_data(train_file_path, tree_name, num_background_events)
X_train, y_train, output_keys = load_classfication_data("classification.root", tree_name, num_background_events)
