...
def load_data(file_path, tree_name, num_events,train=True):
    ...
    print(all_keys)
    input_keys = all_keys1[:12]
    input_keys = input_keys + [all_keys1[15]]
    print(input_keys)
    output_keys = all_keys1[-3:]
    #exit()

    pX, pY, pZ = data['pX'], data['pY'], data['pZ']

    # Define conditions on pX, pY, and pZ
    mask_pX = (pX > -500) & (pX < 500)
    mask_pY = (pY > -100) & (pY < 100)
    mask_pZ = (pZ > -500) & (pZ < 500)

    # Combine the masks
    mask = mask_pX & mask_pY & mask_pZ

    # Apply the mask to all columns in data
    ...
    X = np.column_stack([filtered_data[k] for k in input_keys])
    y = np.column_stack([filtered_data[k] for k in output_keys])
...
WriteRootTree("prediction_poca.root","tree",branchesName,final_output)
